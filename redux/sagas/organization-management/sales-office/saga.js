...
function * list(data) {
  ...
  const receivedData = yield fetch(AppConstants.BASE_HOST_URL + `salesOffice/List`, requestOptions); // Fetch call.
...
function * deleteItem(subIso) {
  ...
  const receivedData = yield fetch(AppConstants.BASE_HOST_URL + `salesOffice/Delete`, requestOptions); // Fetch call.
  const results = yield receivedData.json(); // Convert to JSON.
...
function * get(data) {
  ...
  const receivedData = yield fetch(AppConstants.BASE_HOST_URL + `salesOffice/Get`, requestOptions); // Fetch call.
  const results = yield receivedData.json(); // Convert to JSON.
  console.log(results);
...
function * save(data) {
  ...
  const receivedData = yield fetch(AppConstants.BASE_HOST_URL + `salesOffice/Save`, requestOptions); // Fetch call.
  const results = yield receivedData.json(); // Convert to JSON.
  console.log(results);
